...
public class ChunkManager : MonoBehaviour
{
    ...
    // Modify the existing LoadChunks method as follows
    public void LoadChunks(Vector2Int playerChunkCoordinates)
    {
        ...
        for (int x = playerChunkCoordinates.x - viewDistance; x <= playerChunkCoordinates.x + viewDistance; x++)
        {
            for (int y = playerChunkCoordinates.y - viewDistance; y <= playerChunkCoordinates.y + viewDistance; y++)
            {
                ...
                // Load the chunk if it doesn't exist
                if (!IsChunkLoaded(chunkCoordinates))
                {
    ...
    // Modify the existing UnloadChunks method as follows
    public void UnloadChunks(Vector2Int playerChunkCoordinates)
    {
        List<Vector2Int> loadedChunkCoordinates = new List<Vector2Int>(loadedChunks.Keys);

        // Loop through all loaded chunks
        foreach (Vector2Int chunkCoordinates in loadedChunkCoordinates)
        {
            // Unload the chunk if it's outside the player's view distance
            if (Mathf.Abs(chunkCoordinates.x - playerChunkCoordinates.x) > viewDistance ||
                Mathf.Abs(chunkCoordinates.y - playerChunkCoordinates.y) > viewDistance)
            {
